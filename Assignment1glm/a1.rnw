...
@
<<s3read,echo=FALSE,message=FALSE>>=
@
...
Smoking has been a major health concern for decades for its close association to more than two dozen diseases and conditons, including cancer and heart disease
...
\centering
<<echo=FALSE>>=
  smoke2_summary <- summary(smoke2_fit)$coefficient
  colnames(smoke2_summary)[4] = 'p val.'
  knitr::kable(summary(smoke2_fit)$coefficient,digits=3)
@
